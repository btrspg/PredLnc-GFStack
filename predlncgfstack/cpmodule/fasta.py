...
import collections
from predlncgfstack.utils import eprint
        ...
        if seqID is None:
            for (k, v) in self.seqs.items():
                ...
                while loopSwitch != -1:
                    ...
                    if loopSwitch != -1:
                        fout.write(k + "\t" + str(loopSwitch) + "\t" + str(
                            loopSwitch + length) + "\t" + Pat + "\t0\t+")
                        start = loopSwitch + 1
            ...
            while loopSwitch != -1:
                ...
                fout.write(seqID + "\t" + str(loopSwitch) + "\t" + str(loopSwitch + length) + "\t" + Pat + "\t0\t+")
                start = loopSwitch + 1
        ...
        if rev == True:
            ...
            if seqID is None:
                ...
                while loopSwitch != -1:
                    ...
                    fout.write(seqID + "\t" + str(loopSwitch) + "\t" + str(
                        loopSwitch + length) + "\t" + Pat + "\t0\t-")
                    start = loopSwitch + 1
    ...
    def fetchSeq(self, chr=None, st=None, end=None, infile=None, outfile=None):
        ...
        if (infile is not None) and (outfile is not None):
            ...
            for line in open(infile):
                ...
                if (len(fields) == 3):
                    fout.write(fields[0] + ":" + fields[1] + "-" + fields[2] + "\t" + "strand=+")
                    fout.write(self.seqs[fields[0]][int(fields[1]):int(fields[2])].upper())
                elif (len(fields) > 3):
                    if fields[5] == '-':
                        fout.write(fields[0] + ":" + fields[1] + "-" + fields[2] + "\t" + "strand=-")
                        fout.write(self.seqs[fields[0]][int(fields[1]):int(fields[2])].translate(self.transtab)[
                                   ::-1].upper())
                    else:
                        fout.write(fields[0] + ":" + fields[1] + "-" + fields[2] + "\t" + "strand=+")
                        fout.write(self.seqs[fields[0]][int(fields[1]):int(fields[2])].upper())
            ...
            try:
                ...
                eprint("cannot fetch sequence from " + self.filename + " for " + chr + ":" + str(
                    st) + "-" + str(end))
                return ''
