...
class ORFFinder:
            ...
            if c in start_codens or not start_codens and c not in stop_codens:
                ...
                while True:
                    ...
                    if end:
                        ...
                        if L == self.winner and orf_start < self.result[
                            2]:  # if ORFs have same length, return the one that if upstream
                            # self.winner = L
                            self.result = (direction, frame_number + 1, orf_start, orf_end, L)
    ...
    def longest_orf(self, direction, start_coden=['ATG'], stop_coden=['TAG', 'TAA', 'TGA']):
        if direction == "+":
            ...
            return (self.result[4], self.result[1],
                    self.seq[self.result[2]:self.result[3]])  # CDS length, coding frame, CDS sequence
