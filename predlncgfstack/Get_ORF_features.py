            ...
            if c in start_codens or not start_codens and c not in stop_codens:
                ...
                while True:
                    ...
                    if end:
                        ...
                        if L == self.winner and orf_start < self.longest_orf[
                            1]:  # if ORFs have same length, return the one that if upstream
                            self.winner = L
...
def ORF_features(seq):
    ...
    ORF_frame_score = ((all_orf[0][3] - all_orf[1][3]) ** 2 + (all_orf[0][3] - all_orf[2][3]) ** 2 + (
                all_orf[1][3] - all_orf[2][3]) ** 2) / 2
    ...
    return (mRNA_size, first_orf_len, first_orf_Rlen, longest_orf_len, longest_orf_Rlen, integrity, ORF_frame_score,
            reading_frame_1, reading_frame_2, reading_frame_3, ORF_kmers)
