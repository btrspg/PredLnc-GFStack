...
def Get_ORF_features(input_file):
    ...
    ORF_kmers_result.to_csv("ORF_kmers_features", index=False, sep="\t")
    ...
    tmp = {"transcript_length": transcript_length, "first_orf_len": first_orf_len, "first_orf_Rlen": first_orf_Rlen,
           "longest_orf_len": longest_orf_len, "longest_orf_Rlen": longest_orf_Rlen, "integrity": integrity,
           "ORF_frame_score": ORF_frame_score,
    ...
    result.to_csv("ORF_features", index=False, sep="\t")
...
def get_sequence_features(input_file, model):
    if model == "mouse":
        hex_file = get_resource('Mouse') + '/Mouse_features_Hexamer.tsv'
        # hex_file = "predlncgfstack/Mouse_model/Mouse_features_Hexamer.tsv"
    elif model == 'human':
    ...
    tmp = {"hexamer_score": hexamer, "Instability_index": Instability_index,
           ...
           "transcript_GC1": transcript_GC1, "transcript_GC2": transcript_GC2, "transcript_GC3": transcript_GC3,
           "transcript_GC": transcript_GC, "snr": snr}
    result = pd.DataFrame(tmp)
    result.to_csv("sequence_features", index=False, sep="\t")
    return seq_id
...
def get_special_features(input_file, transcript_length):
    ...
    result = pd.DataFrame(tmp)
    result.to_csv("special_features", index=False, sep="\t")
...
def get_CTD_LncADeep_features(input_file):
    ...
    tmp2 = tmp1.T
    tmp2.to_csv("CTD_LncADeep_features.csv", index=False)
...
def get_all_features(input_file, model):
    ...
    all_X_features.to_csv("All_features", index=False, sep="\t")
    return seq_id
